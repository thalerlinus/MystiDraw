...
const includeChildren = computed(()=> !!page.props.includeChildren);
...
<template>
  <Head :title="'Kategorie Übersicht: ' + (category.name || '')" />
  <AdminLayout :title="'Kategorie: ' + (category.name || '')">
    <template #toolbar>
      <Link href="/admin/categories" class="text-sm text-gray-600 hover:text-gray-800">← Zurück</Link>
    </template>

    <div class="bg-white rounded-xl shadow border border-gray-100">
      <div class="px-4 py-3 border-b flex items-center justify-between">
        <div>
          <h3 class="text-base font-semibold">Produkt-Zuordnung & Status</h3>
          <p class="text-xs text-gray-500">Zeigt Summe in Raffles, vergeben an Nutzer und verbleibend.{{ includeChildren ? ' (inkl. Unterkategorien)' : '' }}</p>
        </div>
